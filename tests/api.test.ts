...
describe('Cart API', () => {
  ...
  it('adds item and returns updated cart', async () => {
    ...
    expect(res.body.items[0]).toMatchObject({ sku: 'ITEM_123', quantity: 2, totalPrice: 20 });
  ...
  it('returns item not found on delete missing item', async () => {
    const cart = await request(app).post('/api/cart');
    const res = await request(app).delete(`/api/cart/${cart.body.cartId}/items/nonexistent`);
  ...
  it('returns cart not found when expired', async () => {
    ...
    const res = await request(app).get(`/api/cart/${cart.body.cartId}`);
    expect(res.status).toBe(404);
    expect(res.body).toEqual({
